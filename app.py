...
# ==========================================
# 🔐 CONFIGURACIÓN DE USUARIOS Y LOGIN DEFINITIVO
# ==========================================
config = {
    "credentials": {
        "usernames": {
            "jesus01": {
                "name": "Jesus",
                "password": "$2b$12$ePTTmgWzaDTnhnWzOFfO..B79SwMWklO2cXrw6U0Sj6Dt7FGwlkZS", # Usa clave: admin123
                "logged_in": False
            },
            "ana02": {
                "name": "Ana",
            ...
            "visitante": {
                "name": "Consultor / Visitante",
                "password": "$2b$12$O8SeldcqtSwNVT/grT3Bp.Pyq9bKJ0Z9jJyS03MpDFcn1/Z.pa5fS", # Usa clave: lector123
                "logged_in": False
            }
        }
    },
    "cookie": {
        "name": "concesiones_cookie",
        "key": "clave_secreta_catastro_bioterra_2026", # Clave única para las cookies de sesión
        "expiry_days": 30
    }
}
...
def registrar_auditoria(usuario, rol, accion):
    try:
        # Creamos la fila con la acción
        nueva_fila = {
        ...
        # Primero cargamos el historial existente para no borrarlo
        df_auditoria_actual = cargar_datos("Auditoria")
        
        # Añadimos la nueva acción
        df_auditoria_nueva = pd.concat([df_auditoria_actual, pd.DataFrame([nueva_fila])], ignore_index=True)
        
        # Guardamos en Google Sheets usando tu API de siempre de forma silenciosa
        df_limpio = df_auditoria_nueva.fillna("")
        datos_json = df_limpio.to_dict(orient="records")
        payload = {"pestana": "Auditoria", "datos": datos_json}
        requests.post(URL_API, json=payload)
        
    except Exception as e:
        print(f"Error silencioso de auditoría: {e}")
...
# ⬇️ DISPARA LA PANTALLA DE LOGIN EN LA PÁGINA ⬇️
# ⬇️ CORRECCIÓN: En las nuevas versiones, .login ya no se asigna a variables
authenticator.login(location='main')

# 🌟 Validamos el estado directamente desde el session_state de Streamlit
if st.session_state["authentication_status"] == False:
    ...
    # Extraemos los datos del usuario logueado desde el session_state
    username = st.session_state["username"]
    name = st.session_state["name"]
    
    # Identificamos su Rol:
    rol_actual = "Lector" if username == "visitante" else "Administrador"
    
    # Colocamos la información del usuario y botón de salida en la barra lateral
    with st.sidebar:
        st.subheader("🔒 Sesión Activa")
        st.markdown(f"👤 **Bienvenido:** {name}")
        st.markdown(f"💼 **Rol:** `{rol_actual}`")
        authenticator.logout('Cerrar Sesión', 'sidebar') # Cambiado a 'sidebar' para que el botón salga abajo del nombre limpio
        st.write("---")

    # ========================================================
    # 🎯 AQUÍ CONTINÚA TODO TU CÓDIGO CON EL TAB RESPECTIVO
    # ========================================================

    # ========================================================
    # 🎯 ¡RECUERDA EL TRUCO DEL TAB!
    # Pega aquí abajo todo el resto de tu código original.
    # Selecciónalo por completo y presiona la tecla 'TAB' una vez
    # para que tenga la sangría alineada dentro de este bloque.
    # ========================================================
